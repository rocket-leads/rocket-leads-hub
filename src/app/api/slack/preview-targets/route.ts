...
/**
 * Admin-triggered preview of the Targets Tracker channel post. Posts to the
 * caller's own DM (not the channel) so the format can be reviewed without
 * spamming the team. Same builder + data as the production cron.
 */
export async function POST(req: NextRequest) {
  ...
  let built
  try {
    built = await buildTargetsTrackerMessage(bodyTemplate)
  } catch (e) {
    return NextResponse.json(
      { ok: false, message: e instanceof Error ? e.message : "Failed to build Targets Tracker message" },
      { status: 500 },
    )
  }
  ...
  try {
    await sendDmToHubUser(
      user.id,
      `_(Targets Tracker channel preview - sent to you only, not to the channel)_\n\n${built.message}`,
    )
    return NextResponse.json({ ok: true, message: "Targets Tracker preview sent to your DM." })
  } catch (e) {
    return NextResponse.json(
