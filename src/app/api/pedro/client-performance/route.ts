 ...
 *  - trend vs the prior equivalent window (7d-vs-prior-7d when days≥7)
 ...
 * /api/clients/[id]/ad-details endpoint uses (5-min revalidate).
 */
...
export async function GET(req: NextRequest) {
  ...
  // ── 1. Resolve client → meta_ad_account_id ──
  ...
  // ── 2. Fetch current + prior windows in parallel (cached) ──
  ...
  // ── 4. Score each ad (winner / loser / neutral) ──
  const scored: ScoredAd[] = adsRaw.map((a) => scoreAd(a, stats.avgCpl))

  // Pre-sorted convenience slices for callers (Pedro UI + future creative-
  // refresh endpoint). Each list is independent so total can overlap.
  ...
  // ── 5. Warnings — flags Claude should respect when reading the output ──
  ...
  // Lead-quality reminder until Phase 3 lead-quality fetch ships
  warnings.push(
