 ...
 * look like a heavy halo). Default radius is `rounded-2xl` (~18px after
...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-5 overflow-hidden rounded-lg bg-card py-5 text-sm text-card-foreground border border-border/60 shadow-[var(--shadow-sm)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-3 data-[size=sm]:py-4 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-lg *:[img:last-child]:rounded-b-lg",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-lg border-t bg-muted/40 p-5 group-data-[size=sm]/card:p-4",
        className
      )}
      {...props}
    />
