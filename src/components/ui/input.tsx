...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <InputPrimitive
      ...
      data-slot="input"
      className={cn(
        // h-10 + rounded-lg + px-3.5 mirrors the Button primitive's default
        // size so inputs and buttons sit on the same baseline when placed
        // next to each other (eg. search + filter, label + save).
