...
- [ ] **Watch List integration** — every "Action Needed" client gets a "Generate fix proposal with Pedro" button in the watchlist UI. One click → Pedro produces the matching stage's output.
...
- [ ] **Per-vertical pattern library** — auto-build, per branche, the catalogue of: winning angles, hooks, scripts, ad-copy patterns. Sourced from RL Meta ads + per-client past deliverables + lead-feedback signals.
- [ ] **Agency-wide insights** — Pedro answers questions like "Welke angles werken het best voor B2B coaches deze quarter?" or "Welk video format heeft de laagste CPL voor renovatie deze maand?". Surfaced as a Pedro Insights tab.
...
- [ ] **Auto-update knowledge base** — when 5+ clients in the same vertical converge on a winning pattern, Pedro proposes a `knowledge/campaigns.md` update. Roy reviews + accepts. Knowledge base self-improves.
- [ ] **AI avatar workflow** — integrate with Heygen/D-ID to actually produce the avatar videos from Pedro's scripts, not just write them. End-to-end pipeline: brief → script → DOCX → avatar video → upload to Drive → ad ready.
- [ ] **Loveable / Manus actuation** — Pedro doesn't just write the LP prompt; he calls Loveable's API and creates the actual landing page. Same for Manus creatives. The CM reviews finished output, not prompts.
