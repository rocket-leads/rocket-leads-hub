# Rocket Leads - Brand Knowledge Base
## Module 1: Tone of Voice & Brand Identity

---

## 1. Bedrijfsidentiteit

**Naam:** Rocket Leads (juridische entiteiten: Rocket Leads Ltd. & Rocket Leads B.V.)
**Opgericht:** 2018
**Oprichter:** Roy Vosters
**Hoofdvestiging (primair, vanaf april 2025):** Rocket Leads Ltd. - 19 Lavele Str., 1000 Sofia, Bulgarije
**Secundaire vestiging:** Rocket Leads B.V. - Jacob van Lennepkade 155H, 1054 ZL Amsterdam, Nederland (optioneel beschikbaar)
**Facturatie:** Alle klanten worden gefactureerd vanuit Rocket Leads Ltd. (Bulgarije)
**Finance e-mail:** Finance@rocketleads.com
**BTW-nummer:** BG VAT BG208169940
**Markt:** Nederland & Vlaanderen (primair), actief in 5+ landen
**Tagline:** *"Your Strategic Partner In Marketing & Sales"* / *"Skyrocket your business"*
**Positionering:** Marktleider leadgeneratie in Nederland - complete marketing- en salesmachine onder één dak

---

## 2. Wat doet Rocket Leads?

Rocket Leads genereert gekwalificeerde afspraken voor bedrijven via een bewezen 3-stappen framework:

1. **Genereer** - Unieke marketingcampagnes (voornamelijk Meta Ads) trekken de juiste prospects aan
2. **Automatiseer** - Volledig geautomatiseerd dashboard met WhatsApp- en e-mailautomatisering
3. **Kwalificeer** - Professionele appointment setters kwalificeren leads voordat ze als afspraak worden doorgezet

**Kernbelofte:** Structureel meer gekwalificeerde afspraken en meer omzet, zonder dat de klant zelf de marketing hoeft te managen.

**Unique selling points:**
- Alles onder één dak: content, advertenties, funnels en opvolging in één systeem
- Één aanspreekpunt, één plan, één team verantwoordelijk voor resultaat
- 100% afspraakgarantie
- No cure no pay beschikbaar
- Klant bepaalt zelf de kwaliteitscriteria van een lead
- Snelle responstijd op verzoeken
- Real-time inzichten via CRM-implementatie

**Bewezen resultaten:**
- 2.000+ bedrijven geholpen
- €75M+ omzet gegenereerd voor klanten
- 8+ jaar ervaring
- 4.7/5.0 gemiddelde beoordeling

---

## 3. Tone of Voice

### Kernkarakter
**Direct. Zelfverzekerd. Resultaatgericht. Licht agressief - maar nooit bot.**

Rocket Leads praat zoals een ambitieuze ondernemer die weet wat hij doet en geen tijd verliest aan fluff. Geen corporaat, geen omslachtig, geen onzekerheid.

### Stem-kenmerken

| Eigenschap | Wel | Niet |
|---|---|---|
| Taal | Nederlands (NL/BE) | Engels tenzij expliciet gevraagd |
| Toon | Direct, zelfverzekerd, scherp | Vaag, wollig, overdreven formeel |
| Energie | Ambitieus, activerend | Voorzichtig, defensief |
| Stijl | Conversationeel maar krachtig | Corporate, droog |
| Bewijs | Concrete cijfers, cases, resultaten | Vage beloftes zonder onderbouwing |
| CTA | Duidelijk en urgent | Vrijblijvend |

### Vaste stijlprincipes

- **Schrijf in jij/jouw** - directe aanspraak, geen afstand
- **Korte zinnen.** Geen lappen tekst.
- **Bullets voor voordelen,** lopende tekst voor verhaal
- **Cijfers zijn heilig** - gebruik ze altijd als ze beschikbaar zijn (€75M+, 2.000+ klanten, 100 leads in eerste maand)
- **Vermijd passieve constructies** - actief, niet lijdend
- **Geen disclaimers** - Rocket Leads staat achter zijn resultaten
- **Urgentie is standaard** - we werken met mensen die willen groeien, niet met mensen die twijfelen

### Toonvoorbeelden

**❌ Fout:**
*"Wij proberen u te helpen bij het genereren van leads via diverse kanalen, wat mogelijk kan resulteren in meer omzet voor uw bedrijf."*

**✅ Goed:**
*"Wij regelen je leads. Jij sluit de deals. Zo simpel is het."*

---

**❌ Fout:**
*"Onze klanten zijn over het algemeen tevreden met de resultaten die we leveren."*

**✅ Goed:**
*"Bijna 200 leads in de eerste maand. 6 maanden later: €100K/maand. Dat is wat Rocket Leads doet."*

---

**❌ Fout:**
*"Mocht u interesse hebben, dan kunt u vrijblijvend contact met ons opnemen."*

**✅ Goed:**
*"Plan een kennismakingsgesprek. We kijken samen of je in aanmerking komt."*

---

## 4. Doelgroep (ICP - Ideal Customer Profile)

**Primaire doelgroep:** Ambitieuze Nederlandse/Vlaamse bedrijfseigenaren die:
- Willen schalen maar de marketing niet zelf willen of kunnen managen
- Moe zijn van afhankelijkheid van losse freelancers of wisselende bureaus
- Resultaat willen, geen excuses
- Beschikbaar zijn voor afspraken en snel kunnen handelen

**Sectoren (bewezen cases):**
- Energiesector (warmtepompen, zonnepanelen, isolatie)
- Bouw & installatie (aannemers, schilders, stukadoors)
- B2B dienstverlening
- Recruiters/uitzendbureau's
- Marketingbureaus die willen groeien
- Lokale dienstverleners (SME)

**Psychografisch profiel:**
- Groeimentaliteit, ongeduldig met slechte resultaten
- Wil marktleider worden in zijn niche
- Heeft eerder slechte ervaringen gehad met andere bureaus
- Wil één partner die alles regelt, geen 5 losse partijen
- Begrijpt dat marketing een investering is, geen kostenpost

**Wat ze NIET willen:**
- Nog een bureau dat praat maar niet levert
- Onduidelijke rapportages
- Leads van slechte kwaliteit
- Lange onboarding processen

---

## 5. Merkwaarden

1. **Resultaat boven alles** - we worden afgerekend op output, niet op inspanning
2. **Ownership** - we nemen verantwoordelijkheid, we schuiven het niet af
3. **Snelheid** - we handelen snel, zowel intern als voor klanten
4. **Expertise** - 8+ jaar, €75M+ bewezen resultaat, geen experimenten met het geld van klanten
5. **Partnerschap** - niet een leverancier, maar een groeipartner

---

## 6. Visuele identiteit (richtlijnen)

- **Primaire kleur:** Rood/oranje (raketenergie, urgentie, ambitie)
- **Stijl:** Modern, clean, professioneel maar niet koud
- **Content:** Veel gebruik van video en foto (teamcontent, resultaatcontent, social proof)
- **Logo:** Rocket Leads cirkellogo + horizontaal tekstlogo

---

## 7. Organisatiestructuur

| Naam | Rol |
|---|---|
| **Roy Vosters** | Founder & Adviseur - strategisch niveau, AI-implementatie |
| **Mike** | CEO / Country Manager - front-end (marketing, sales) |
| **Roel** | CEO / Country Manager - back-end (delivery, finance, operations) |
| **Stefan** | Campaign Manager - Team 2 |
| **Danny** | Account Manager - Team 2 |
| **Quintus** | Appointment Setter |
| **Shanna** | Graphic Designer & Content |
| **Anel** | Closer (ervaren) |
| **Jill** | Closer (in ontwikkeling, leert van Roy & Anel) |
| **Arno** | Finance - facturen, betalingen, cashflow, debiteurenbeheer |

**Delivery structuur:** 2 teams
- Team 1: Mike + Roel
- Team 2: Stefan + Danny

---

## 8. Sales & Calendly

- **Type calls:** Kennismakingsgesprek / Groei Call
- **Calendly event naamgeving:** Bevat altijd "Kennismaking" of "Groei call"
- **Closers:** Anel (senior), Jill (junior, in training)
- **Verkoopbenadering:** Direct, resultaatgericht, op basis van social proof en bewezen cases

---

## 9. Marketingkanalen

| Kanaal | Verantwoordelijke | Status |
|---|---|---|
| Meta Ads (primair) | Mike | Hoofdbron van leads |
| Google Ads | Mike | Actief |
| Instagram organic | Mike | Actief |
| LinkedIn organic | Mike | Actief |
| E-mailmarketing | Mike | Voornamelijk reactivatie |

---

## 10. Veelgebruikte claims & social proof (te gebruiken in copy)

- "2.000+ bedrijven geholpen"
- "€75M+ omzet gegenereerd voor onze klanten"
- "8+ jaar bewezen resultaat"
- "4.7/5.0 beoordeling"
- "100% afspraakgarantie"
- "Marktleider leadgeneratie Nederland"
- "Bijna 200 leads in de eerste maand" (Luc van Elsloo, Vlex Vending)
- "100+ kwalitatieve leads per maand" (Vincent Stoit, UPTMZ)
- "Van wisselende resultaten naar consistente groei" (DiamondFlame)

---

*Versie 1.0 - Aangemaakt: maart 2026*
*Volgende module: ICP & Doelgroepanalyse*
